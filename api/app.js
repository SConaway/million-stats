...
async function fetchOldest(id) {
	try {
		...
		let number;
		for (let x = 2; x < result.messages.length; x++) {
			console.log(
				`x: ${x}; message=${JSON.stringify(
					result.messages[result.messages.length - x],
				)}`,
			);
			number = extractNumber(
				result.messages[result.messages.length - x].text,
			);
...
async function report() {
	...
	let message = `Good evening, my underlings! You have done a _brilliant_ job with the counting. Mmm. Yes. Numbers are savory. 
